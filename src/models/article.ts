...
import { ARTICLE_CATEGORIES, ARTICLE_KINDS, ARTICLE_STATUSES, type ArticleKind, type ArticleStatus } from "@/lib/article-types";
...
const articleSchema = new Schema<ArticleDocument>({
  locale: { type: String, enum: ["vi"], required: true, default: "vi" },
  ...
  tags: { type: [String], required: true, validate: { validator: (tags: string[]) => tags.length <= 12 && tags.every((tag) => tag.length <= 40), message: "Tags không hợp lệ" } },
  summary: { type: String, required: true, trim: true, maxlength: 500 },
  content: { type: String, required: true, maxlength: 500_000 },
}, { timestamps: true, versionKey: false });

articleSchema.index({ locale: 1, kind: 1, slug: 1 }, { unique: true });
articleSchema.index({ locale: 1, kind: 1, status: 1, date: -1 });
