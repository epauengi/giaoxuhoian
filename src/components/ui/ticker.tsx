...
export function Ticker({
  ...
  const row = (copy = false) => (
    <div aria-hidden={copy || undefined} className={cn("flex shrink-0 items-center", copy && "ticker-copy")}>
      {items.map((item) => (
        <span key={`${copy ? "copy" : "item"}-${item}`} className="ticker-item flex items-center">
          <span className="mx-4 shrink-0 bg-accent px-2 py-0.5 font-mono text-xs font-medium uppercase tracking-widest text-paper">
            {ticker.label}
          </span>
          <span className="font-sans text-sm">{item}</span>
          <span aria-hidden className="ticker-bullet mx-6 text-neutral-500">✦</span>
        </span>
      ))}
    </div>
  );
